...
public class GroupTest {
    @Test void testNoGroups(){
        ...
        assertEquals(grid.getAllGroupsOfColour(Color.RED).size(), 0);
    }

    @Test void testOneGroup(){
        Grid grid = makeMockup(3,1,0,1,1);
        Game game = new Game(grid);
        assertEquals(grid.getAllGroupsOfColour(Color.RED).size(), 1);    }
    ...
    @Test void testRedGrid_blue(){
        Grid grid = makeMockup(3,100,0,1,1);
        Game game = new Game(grid);
        assertEquals(grid.getAllGroupsOfColour(Color.BLUE).size(), 0);    }
    ...
    @Test void testTwoGroups(){
        ...
        Game game = new Game(grid);
        assertEquals(grid.getAllGroupsOfColour(Color.RED).size(), 2);    }
    ...
    @Test void testRealGrid_red(){
        ...
        Grid grid = specificMockup(2, indices);
        Game game = new Game(grid);
        assertEquals(grid.getAllGroupsOfColour(Color.RED).size(), 3);    }

    @Test void testRealGrid_blue(){
        int[] indices = {0,0,2,2,1,1,2,2,2,2,0,0,1,1,2,1,2,0,0};
        Grid grid = specificMockup(2, indices);
        Game game = new Game(grid);
        assertEquals(grid.getAllGroupsOfColour(Color.BLUE).size(), 3);    }

    @Test void testDifferentGroups(){
        int[] indices = {0,2,0,2,2,2,2,0,0,0,2,0,2,2,2,2,0,0,0};
        Grid grid = specificMockup(2, indices);
        Game game = new Game(grid);
        assertEquals(grid.getAllGroupsOfColour(Color.RED).size(), 5);    }
    ...
    @Test void testWebsiteExample(){
        ...
        Game game = new Game(grid);
        assertEquals(grid.getAllGroupsOfColour(Color.BLUE).size(), 10);    }
